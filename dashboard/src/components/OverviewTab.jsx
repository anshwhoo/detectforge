...
export default function OverviewTab({ rules, attackLayer, runs, buildProgress, systemHealth, coverageHistory, setActiveTab }) {
  ...
  // "Last CI Run" means the CI workflow specifically (rule lint/convert/test), not
  // whatever workflow run happens to be newest - pipeline_runs.json mixes in CD and
  // dashboard-deploy runs too. It also only considers completed runs: this data is
  // generated by the dashboard-deploy workflow querying its own run history mid-flight,
  // so the newest entry is frequently itself, still in_progress with conclusion: null,
  // which read as "failed" even though nothing had actually failed yet.
  const ciRuns = runs.filter((r) => r.name === 'DetectForge CI — Lint, Convert & Test Rules' && r.status === 'completed');
